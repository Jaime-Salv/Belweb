...
function ensurePushStyles() {
  ...
  style.textContent = `
    .push-notification-btn {
      flex: 0 0 auto;
      border: 1px solid rgba(119,80,90,.15);
      border-radius: 12px;
      padding: 9px 11px;
      background: #fff;
      color: #2b2325;
...
async function saveSubscription(subscription) {
  ...
  const { error } = await supabasePush.from('push_subscriptions').upsert({
...
async function activatePushNotifications() {
  ...
  try {
    ...
    console.error(error);
    button.textContent = '⚠️ No se pudo activar';
    setTimeout(() => {
      button.textContent = '🔔 Activar avisos';
      button.disabled = false;
    }, 2500);
...
async function sendAssistancePush() {
  ...
  const { data: members, error } = await supabasePush
    .from('couple_members')
    .select('id,display_name');
  if (error) return;

  const me = members.find(member => member.id === session.user.id);
  const other = members.find(member => member.id !== session.user.id);
  if (!me || !other) return;
  ...
  try {
    await fetch('/api/send-push', {
      ...
      body: JSON.stringify({
        ...
        title: 'Bel & Jaime ❤️',
        message: `🚨 ${me.display_name.toUpperCase()} NECESITA ASISTENCIA MIMAL`,
        url: '/#emergencySection',
...
const assistanceButton = document.querySelector('#startBtn');
assistanceButton?.addEventListener('click', sendAssistancePush, true);

supabasePush.auth.onAuthStateChange((_event, session) => {
  currentSession = session;
  setTimeout(updateButtonState, 0);
});

const { data: { session } } = await supabasePush.auth.getSession();
currentSession = session;
